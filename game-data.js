...
/* 선수 포지션 — 팀명+등번호로 찾는다(seasonOf·recentOf와 같은 키다).
   박스스코어에는 포지션이 없다: 기록지는 숫자만 적는 종이라 포지션을 받지 않는다.
   그런데 카드의 실루엣은 포지션으로 갈리므로, 명부에 있는 값을 여기 한 벌 둔다.
   운영 콘솔(index.html)의 PLAYERS가 원본이고 이건 공개 화면이 읽는 사본이다. */
var BASKET_POS = {
  '서울 타이탄스|7':'SF',
  '서울 타이탄스|11':'PG',
  '서울 타이탄스|23':'C',
...
var BASKET = {
  ...
  /* 포지션 — 없으면 빈 문자열. 없는 값을 지어내지 않는다(카드는 그때 예비 실루엣으로 떨어진다). */
  posOf: function(teamName, no){ return BASKET_POS[teamName + '|' + no] || ''; },
